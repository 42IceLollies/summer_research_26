...
def ctmd(d, l, depth, base, name, options):
...
def moov(b, d, l, depth, options):
  if not options["quiet"]:
    print('moov: (0x%x)'%l)

def uuid(b, d, l, depth, options):
  uuidValue = d[:16]
  if not options["quiet"]:
    print('{1}uuid: {0} (0x{2:x})'.format(hexlify(uuidValue), '', l))
  return uuidValue  

def stsz(b, d, l, depth, options):
...
def cncv(b, d, l, depth, options):    
  if not options["quiet"]:
    print('CNCV: {0} (0x{1:x})'.format(d, l) ) 
  return d[:]

def cdi1(b, d, l, depth,options):    
  if not options["quiet"]:
...
def parse(offset, d, base, depth, options):
  o = 0
  ...
  while o < len(d):
    ...
    if chunkName in tags: #dedicated parsing
      r = tags[chunkName](base, d[o+no:o+no +l-no], l, depth+1, options) #return results
      ...
      r = ctmd( d[o+no:o+no +l-no], l, depth+1, base+o+no, chunkName, options)
      cr3[ chunkName ] = r 
    else:
      if not options["quiet"]:
    ...
    if chunkName in { b'moov', b'trak', b'mdia', b'minf', b'dinf', b'stbl' }: #requires inner parsing, just after the name
      parse( offset+o+no, d[o+no:o+no +l-no], base+o+no, depth+1, options)
      ...
      if uuidValue == unhexlify('85c0b687820f11e08111f4ce462b6a48') or uuidValue == unhexlify('5766b829bb6a47c5bcfb8b9f2260d06d') or uuidValue == unhexlify('210f1687914911e4811100242131fce4'):
        parse(offset+o+no+UUID_LEN, d[o+no+UUID_LEN:o+no+UUID_LEN +l-no-UUID_LEN], base+o+no+UUID_LEN, depth+1, options)
      elif uuidValue == unhexlify('eaf42b5e1c984b88b9fbb7dc406e4d16'):
        parse(offset+o+no+UUID_LEN+8, d[o+no+UUID_LEN+8:o+no+UUID_LEN+8 +l-no-8-UUID_LEN], base+o+no+UUID_LEN+8, depth+1, options)
      ...
      parse( offset+start, d[start:end], start, depth+1 , options)
...
def parse_image(dat):
  ...
  if data[4:12]==b'ftypheix' or data[4:12]==b'ftypcrx ':
    offset = parse(0, data, 0, 0, options)
    if options["verbose"]>0:
  ...
  return {"cr3": cr3, "big_crx": big_crx, "small_crx": small_crx, }